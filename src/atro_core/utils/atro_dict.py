...
def merge_dicts(
    ...
    if current_name and updating_dict_name:
        LOG.debug(f"Updating {current_name} with {updating_dict_name} with overwrite set to {overwrite}.")
    elif current_name:
        LOG.debug(f"Updating {current_name} with overwrite set to {overwrite}.")
    else:
        LOG.debug(f"Updating dictionary with overwrite set to {overwrite}.")
    ...
    if all([isinstance(key, str) for key in current_dict.keys()]) and all(
        ...
        if len(only_diff) > 0:
            LOG.debug("The following changes were made:")
            LOG.debug("\n".join(only_diff))
        else:
...
def merge_dict_with_yaml(
    ...
    if not yaml_path.exists():
        LOG.debug(f"File in {yaml_path.as_posix()} does not exist, skipping.")
        return current
