...
from atro_core.args.arg_casting import cast_dict_based_on_args
from atro_core.args.arg_source import ArgSource
from atro_core.args.class_populating import add_dataclass, add_pydantic, get_dataclass, get_pydantic
from atro_core.args.helpers import get_duplicates, restrict_keys, throw_if_required_not_populated
from atro_core.args.source_loading import load_source
from atro_core.utils import merge_dicts
...
class InputArgs(BaseModel):
    ...
    def validate_sources(self) -> None:
        if len(set(self.sources)) != len(self.sources):
            ...
            raise ValueError(
                "The elements of list sources must be unique. The following elements are duplicated: "
                + ", ".join(dupes)
                + "."
            )
    ...
    def validate_args(self) -> None:
        ...
        if len(set(names)) != len(names):
            ...
            raise ValueError(
                "Can't have two or more Args with the same name parameter. The following names are duplicated: "
                + ", ".join(dupes)
                + "."
            )
    ...
    def add(
        ...
        self.add_arg(
            Arg(name=name, other_names=other_names, arg_type=arg_type, help=help, required=required, default=default)
        )
