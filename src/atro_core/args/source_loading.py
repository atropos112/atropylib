...
def load_source(
    source: ArgSource | Path, prefix: str, args: Sequence[Arg], cli_input_args: Sequence[str] | None = None
) -> dict[str, Any]:
    ...
    match source:
        ...
        case _:
            match source.name.split(".")[-1]:  # source.suffix would not work here as .env would map to empty string
                ...
                case _:
                    raise Exception(
                        f"Tried loading {source} of type {type(source)} but failed. File type '{source.suffix}' is not supported."
                    )
...
def get_env_args(prefix: str, args: Sequence[Arg]) -> dict[str, str]:
    ...
    for arg in args:
        ...
        # If arg name is not in the envs, maybe one of the other names is
            ...
            other_nms = [
                other_name.lower() for other_name in arg.other_names if other_name.lower() in all_prefixed_envs
            ]
            if other_nms:
