...
# region casting
def cast_to(s: Any, arg_type: type[T]) -> T:
    # If type is correct return as is
    if isinstance(s, arg_type) or arg_type == Any:
    ...
    if arg_type in [Mapping, Sequence, list, dict]:
        if not isinstance(s, str):
            raise ValueError(
                f"Could not load {s} as {arg_type} because it is not clear how to load type {type(s)} into {arg_type}."
            )
