    ...
    for key, val in pydantic_class_type.model_fields.items():
        ...
        output.append(
            Arg(
                ...
                arg_type=val_type,
                required=required,
                default=None if str(val.default) == "PydanticUndefined" else val.default,
            )
        )  # type: ignore
...
def get_dataclass(
    typed_dict: dict[str, Any], dataclass_type: type[T], cli_input_args: Sequence[str] | None = None
) -> T:
    if not is_dataclass(dataclass_type):
        raise Exception(
            f"Developer error: '{dataclass_type}' is not a dataclass and so it shouldn't call __get_dataclass."
        )
...
def get_pydantic(
    typed_dict: dict[str, Any], pydantic_class_type: type[T], cli_input_args: Sequence[str] | None = None
) -> T:
    if not issubclass(pydantic_class_type, BaseModel):
        raise Exception(
            f"Developer error: '{pydantic_class_type}' is not a subclass of 'BaseModel' and so it shouldn't call __get_pydantic."
        )
    model_args_required = pydantic_class_type.model_fields  # type: ignore

    return get_cls_setup(typed_dict, pydantic_class_type, model_args_required, cli_input_args=cli_input_args)  # type: ignore
...
def get_cls_setup(
    typed_dict: dict[str, Any], cls: type[T], model_args_required: dict, cli_input_args: Sequence[str] | None = None
) -> T:
